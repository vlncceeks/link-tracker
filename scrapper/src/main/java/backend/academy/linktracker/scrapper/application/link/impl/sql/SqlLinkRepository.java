...
public class SqlLinkRepository implements LinkRepository {
    ...
    public TrackedLink add(Long chatId, TrackedLink link) {
        return jdbcClient.sql("""
            ...
            .param("lastCheckedAt", link.getLastCheckedAt() != null
                ? link.getLastCheckedAt() : Instant.now())
            .query(this::mapRow)
            .single();
    }
    ...
    public void remove(Long chatId, String url) {
        jdbcClient.sql("DELETE FROM tracked_links WHERE chat_id = :chatId AND url = :url")
            .param("chatId",chatId)
            .param("url", url)
            .update();
    }
    ...
    private TrackedLink mapRow(ResultSet rs, int rowNum) throws SQLException {
        ...
        Set<String> tags = tagsArray != null
            ? new HashSet<>(Arrays.asList((String[]) tagsArray.getArray()))
            : new HashSet<>();
        ...
        link.setLastCheckedAt(rs.getObject("last_checked_at", Instant.class));
        return link;
    }
